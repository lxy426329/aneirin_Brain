...
```
             MCP 客户端（Claude / Cline / Cursor ...）
                          ...
                          ▼
             ┌──────────────────────┐
             │      server.py       │  47 个 MCP 工具 + HTTP API
             └──────┬───────────┬───┘
                    │           │
          ┌─────────▼───┐   ┌───▼──────────┐
...
│   ├── voice/                        # 说话习惯/称呼/相处方式（breath 每次强制注入）
...
| `voice/` | 说话方式层。说话习惯、称呼、相处方式，**每次 breath() 无论有无 query 都强制注入**，不走评分、不走衰减 |
...
| `echo_chamber/` | 回音壁。管家工作总结（每日/每周摘要）与待审批提案（清理/冲突/合并/身份收录），主 AI 用 approve_action/reject_action 终审 |
...
| `voice` | voice/ | 说话方式（breath 每次强制注入，不衰减） |
| `anchor` | anchor/ | 行为锚点 |
...
| 工具 | 设计用途 |
|---|---|
| `hold` | 存储单条记忆，自动情感打标 + 相似查重合并。pinned/protected/feel/task_flag/source/title/显式 valence-arousal 可控 |
| `grow` | 日记归档：一段长内容自动拆分为多桶（<30 字走 hold 快速路径） |
| `breath` | 检索/呼吸：无 query=被动浮现（最近未解决+权重 TOP-3）；带 query=三步检索管线（强锚点→年轮经验→五维加权混合检索+Rerank 硬截断 Top3） |
| `trace` | 修改记忆元数据/内容：resolved 沉底、pinned 钉选、digested 隐藏、task_flag、content 替换、delete=True 删除。主 AI 执行日终决策的主要写工具 |
| `dream` | 做梦自省：读取最近新增记忆供 AI 自省；读后可 trace(resolved=1) 放下或 hold(feel=True) 写感受 |
...
| `approve_action` / `reject_action` | 批准/驳回提案（cleanup 删向量、conflict 标记、chain_merge 合并实体、identity 建档案；批准即执行） |
...
| 后台任务 | 触发 | 作用 |
|---|---|---|
| `decay_engine` | 懒启动，默认 24h 间隔 | 艾宾浩斯遗忘曲线衰减、自动归档、自动结案；进程内文件锁互斥 |
