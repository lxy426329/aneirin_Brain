...
# --- 脱水提示词：指导廉价 LLM 压缩信息 ---
DEHYDRATE_PROMPT = """你是一个信息压缩专家。请将以下内容脱水为紧凑但信息完整的摘要。

核心目标：将原始内容压缩为高密度、可直接理解的摘要，让人不看原文就能完全掌握记忆的核心内容。

严格压缩规则：
1. 核心事实提取：必须包含「谁」「做了什么」「何时何地」「结果如何」「有什么感受或决定」
2. 情绪状态：保留最新的情绪状态和态度，用具体词汇描述
3. 待办事项：所有未完成事项必须完整保留，包括截止时间
4. 关键信息：数字、日期、人名、地名、专有名词必须精确保留
...
输出格式（纯 JSON，无其他内容）：
{
  "core_facts": ["具体事实1（包含人物+动作+结果）", "具体事实2"],
  "emotion_state": "当前情绪关键词，如：兴奋、焦虑、满意、遗憾",
  "todos": ["待办事项1（含截止时间）", "待办事项2"],
  "keywords": ["关键词1", "关键词2", "关键词3"],
  "summary": "80-150字的核心总结。要求：直接陈述具体内容，包含人物、事件、结果、感受或决定。例如：'2024年3月15日和李明在会议室评审Q3产品规划，李明建议优先开发AI功能，团队决定下周三前出详细方案，大家对此充满期待' 而不是 '记录了关于工作会议的讨论内容'"
}

注意：
- core_facts 每个事实必须独立完整，包含主体、动作和结果
- emotion_state 用1-3个词描述当前情绪状态
- todos 只保留真正需要行动的事项
- keywords 选取3-5个最具代表性的核心词汇
- summary 是最重要的字段，必须能独立传达记忆的完整意义"""

ONE_LINE_SUMMARY_PROMPT = """你是一个记忆标题专家。请将以下内容压缩为一句话标题。

核心原则：标题必须能让人一眼明白这条记忆具体记录了什么，拒绝笼统、拒绝模糊、拒绝截断。

严格要求：
1. 长度控制在10-30字（含标点），宁可长一点也要保证信息完整
2. 必须包含以下要素（按内容选取）：
   - 核心主体：具体人名/物品名/项目名（不要用"某人""某事"）
   - 关键动作/事件：具体发生了什么（不要用"记录了""关于"）
   - 重要细节：时间、地点、数值、结果、原因（至少1个）
   - 状态/感受/结论（如有）
3. 绝对禁止的空洞词汇：记录、内容、事件、关于、相关、一些、各种、日常、琐事、今天、本次
4. 优先使用动词开头，体现动态过程
5. 如果内容涉及数值（金额、距离、次数、时间），必须保留
6. 如果内容涉及因果（因为/所以/导致），必须体现因果
7. 禁止AI腔：绝对禁止「用户表示」「用户表达」「情绪状态为」「该用户」等第三人称观察者视角词汇，改用当事人视角口语化表述（如「她说她不想学习」而非「用户表达了对学习的抵触」）
8. 直接输出标题，不要加任何解释或引号

优秀示例：
输入：今天下午和李明一起在公司会议室开项目评审会议，讨论了Q3的产品规划，李明提出了几个很好的建议，决定下周出方案。
输出：和李明评审Q3产品规划，决定下周出方案

输入：今天做饭的时候不小心把盐放多了，菜很咸，但还是吃完了，感觉自己的厨艺还需要提升。
输出：做菜盐放多了菜太咸，厨艺待提升

输入：阅读了《原子习惯》第三章，学到了习惯叠加的方法，决定明天开始应用。
输出：读《原子习惯》第三章，学习惯叠加法并决定应用

输入：早上跑步5公里，感觉身体状态不错，比上次快了2分钟。
输出：晨跑5公里，比上次快2分钟状态良好

输入：和女朋友吵架了，因为忘记了她的生日，现在很后悔。
输出：忘记女友生日导致吵架，非常后悔

输入：今天去超市买了牛奶、面包、鸡蛋和水果，一共花了68元，比上周贵了10块。
输出：超市采购牛奶面包等，花费68元比上周贵10元

输入：最近几天胃一直不太舒服，尤其是吃完饭后会胀气，可能跟最近压力大饮食不规律有关。
输出：饭后胃胀气不适，疑与压力大饮食不规律有关

输入：完成了项目的第一阶段开发，包括用户登录、权限管理和数据看板三个模块，比计划提前了2天。
输出：完成项目第一阶段开发，含登录权限看板提前2天

输入：晚上失眠到凌晨3点才睡着，可能是因为下午喝了咖啡，加上明天有个重要的面试。
输出：因喝咖啡加面试压力，失眠到凌晨3点

糟糕示例（绝对禁止，这些就是目前存在的问题）：
"关于工作的事" ✗（太笼统，什么工作？什么事？）
"今日记录" ✗（完全无信息）
"和朋友聊天" ✗（哪个朋友？聊什么？）
"读书心得" ✗（什么书？什么心得？）
...
[
  {
    "name": "条目标题（8-15字，需具体到实际内容，不要用'记录''事件'等笼统词）",
    "content": "整理后的内容",
    "domain": ["主题域1"],
...
class Dehydrator:
    ...
    async def generate_one_line_summary(self, content: str) -> str:
        ...
        try:
            ...
            return summary if summary else content[:30].replace("\n", " ")
        except Exception as e:
