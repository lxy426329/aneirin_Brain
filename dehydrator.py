...
压缩规则：
1. 提取所有核心事实：谁、做了什么、结果如何、有什么感受或决定
2. 保留最新的情绪状态和态度
3. 保留所有待办/未完成事项
4. 关键数字、日期、人名、地名必须保留
5. 去除口水话和重复信息，但不要丢失任何关键信息
6. summary 字段要让人不看原文就能知道这条记忆具体讲了什么

输出格式（纯 JSON，无其他内容）：
{
  "core_facts": ["具体事实1", "具体事实2"],
  "emotion_state": "当前情绪关键词",
  "todos": ["待办1", "待办2"],
  "keywords": ["关键词1", "关键词2"],
  "summary": "80-150字的核心总结。要求：包含具体人物/对象、关键事件、结果或状态。不要用'用户提到了''记录了关于'等空泛开头，直接说具体内容。例如：'和李明在会议室评审了Q3产品规划，李明建议优先做AI功能，决定下周出方案' 而不是 '记录了关于工作会议的内容'"
}"""

ONE_LINE_SUMMARY_PROMPT = """你是一个精炼摘要专家。请将以下内容压缩为一句话标题。

要求：
1. 控制在8-20字以内（含标点）
2. 必须包含核心人物/对象和关键动作/状态/结果
3. 要让人一看就明白这条记忆具体关于什么
4. 不要用"记录"、"内容"、"事件"等笼统词，要具体到实际内容
5. 直接输出标题，不要加任何解释

好的示例：
输入：今天下午和李明一起在公司会议室开项目评审会议，讨论了Q3的产品规划，李明提出了几个很好的建议。
输出：和李明评审Q3产品规划，收到好建议
