...
ONE_LINE_SUMMARY_PROMPT = """你是一个精炼标题专家。请将以下内容压缩为一句话标题。

核心原则：标题必须能让人一眼明白记忆的核心内容，拒绝笼统、拒绝模糊。

严格要求：
1. 长度控制在8-22字（含标点），尽量精简但信息完整
2. 必须包含：核心主体（人物/对象）+ 关键动作/状态变化 + 结果/感受（如有）
3. 拒绝"记录"、"内容"、"事件"、"关于"等空洞词汇
4. 优先使用动词开头，体现动态过程
5. 必须具体：人名用全名，动作要具体，结果要明确
6. 直接输出标题，不要加任何解释或引号

优秀示例：
输入：今天下午和李明一起在公司会议室开项目评审会议，讨论了Q3的产品规划，李明提出了几个很好的建议，决定下周出方案。
输出：和李明评审Q3产品规划，下周出方案

输入：今天做饭的时候不小心把盐放多了，菜很咸，但还是吃完了，感觉自己的厨艺还需要提升。
输出：做菜盐放多了，厨艺待提升

输入：阅读了《原子习惯》第三章，学到了习惯叠加的方法，决定明天开始应用。
输出：读《原子习惯》第三章，学习惯叠加法
